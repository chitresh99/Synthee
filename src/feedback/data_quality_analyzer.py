...
class DataQualityAnalyzer:
    ...
    def _load_csv_robust(self):
        try:
            ...
            try:
                ...
                print(f"CSV loaded with bad lines skipped: {df.shape[0]} rows and {df.shape[1]} columns")
                return df
            except:
                pass
            ...
            try:
                ...
                print(f"CSV loaded with auto-detected separator: {df.shape[0]} rows and {df.shape[1]} columns")
                return df
            except:
                pass
            ...
            try:
                ...
                print(f"CSV loaded with no quoting: {df.shape[0]} rows and {df.shape[1]} columns")
                return df
            except:
                pass
            ...
            try:
                ...
                print(f"CSV loaded after manual fixing: {df.shape[0]} rows and {df.shape[1]} columns")
                return df
        ...
        for i, line in enumerate(lines[1:], 2):
            ...
            if field_count != expected_columns:
                ...
                if len(problematic_lines) <= 5: 
                    print(f"Line {i}: has {field_count} fields (expected {expected_columns})")
    ...
    def check_consistency(self):
        ...
        for col in categorical_cols:
            if self.df[col].dtype == 'object':
                ...
                spaces = (self.df[col].str.len() != self.df[col].str.strip().str.len()).sum()
                if spaces > 0:
                    issues.append(f"{col}: {spaces} values with leading/trailing spaces")
        
        numeric_cols = self.df.select_dtypes(include=[np.number]).columns
        for col in numeric_cols:
            if any(keyword in col.lower() for keyword in ['price', 'cost', 'amount', 'quantity', 'count', 'age', 'rating']):
    ...
    def check_value_ranges(self):
        ...
        for col in numeric_cols:
            ...
            elif any(keyword in col_lower for keyword in ['price', 'cost']) and min_val < 0:
                range_issues.append(f"{col}: Negative prices found")
            elif 'rating' in col_lower and (min_val < 0 or max_val > 10):
                range_issues.append(f"{col}: Rating range {min_val}-{max_val} outside 0-10 scale")
            elif any(keyword in col_lower for keyword in ['percent', 'rate', '%']) and (min_val < 0 or max_val > 100):
    ...
    def calculate_quality_scores(self, consistency_issues, range_issues):
        ...
        uniqueness_score = min((self.df.nunique().sum() / (len(self.df) * len(self.df.columns))) * 100, 100)
        consistency_score = max(0, 100 - len(consistency_issues) * 10)
        range_score = max(0, 100 - len(range_issues) * 15)
        overall_score = (completeness + uniqueness_score + consistency_score + range_score) / 4
    ...
    def generate_recommendations(self, quality_assessment, consistency_issues, range_issues, quality_scores):
        recommendations = []
        rows = len(self.df)
        
        if rows < 1000:
            recommendations.append(f"Increase dataset size to 1000+ rows (current: {rows})")
        ...
        low_variety_cols = [col for col in self.df.columns 
                           if self.df[col].nunique() / len(self.df) < 0.1 and self.df[col].nunique() > 1]
        if low_variety_cols:
            recommendations.append(f"Increase variety in: {', '.join(low_variety_cols)}")
    ...
    def run_full_analysis(self):
        ...
        recommendations = self.generate_recommendations(quality_assessment, consistency_issues, range_issues, quality_scores)
