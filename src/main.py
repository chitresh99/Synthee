...
def main():
    ...
    if not generate_available:
        st.error("Cannot proceed without the generation module. Please check your imports.")
        return
    ...
    with col2:
        ...
        if hasattr(st.session_state, 'generated_csv') and st.session_state.generated_csv:
            # Download button
            st.download_button(
                label="💾 Download CSV",
    ...
    if hasattr(st.session_state, 'show_results') and st.session_state.show_results:
        ...
        with tab1:
            try:
                ...
                with col3:
                    st.metric("Data Size", f"{len(st.session_state.generated_csv)} chars")
